...
public class PaperExcelServiceImpl implements PaperExcelService {
    ...
    */
    ...
    public JsonResult deleteByYearAndMonth(Integer year, Integer month, Integer type) {
        ...
        paperManager.deleteByYearAndMonth(time.get(), paperTypeManager.createPaperType(type));
...
//    }
    ...
    private Paper getNewAndChangeOld(final Integer type, final Time time, final Category category, final PaperExcelModel model) {
        ...
        Paper tempPaper = paperManager.findByAccessionNumberAndTime(model.getAccessionNumber(), time).orElse(null);
        if (tempPaper == null) {
            //说明数据库中没有
            paper = PaperConverter.convertExcelPaperToEntity(model, category, time, paperTypeManager.getPaperType(type));
        } else {
            ...
            paper.setPaperType(paperTypeManager.changePaperType(type, tempPaper.getPaperType()));
